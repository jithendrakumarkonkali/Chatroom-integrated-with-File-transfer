...
class ClientApp:
    def __init__(self, root, server_ip_arg=None):
        ...
        self.setup_ui()
        # No auto-connect on init, user must click connect.
        # If server_ip_arg was provided, it's pre-filled.
    ...
    def handle_user_disconnected(self, username_disconnected):
        if username_disconnected in self.conversations:
            # Optionally, add a system message to their conversation log
            # self.conversations[username_disconnected].append(f"[SYSTEM] {username_disconnected} has disconnected.\n")
    ...
    def handle_file(self, data):
        try:
            ...
            if len(filedata_received) != filesize: # Check if all data is there
                # This basic check might not be enough for very large files if not all data was part of `parts[3]`
                # due to split limit. Assuming current protocol sends all file data after 4th colon.
                # For robust large file transfer, a streaming approach or careful chunking protocol is needed.
                # For this exercise, assuming filesize is for data after the 4th part.
    ...
    def display_message_in_chat(self, message, tag, sender_is_originator=None):
        # This method is for messages that should appear if the chat is active
        # sender_is_originator is the user who sent the message/file
        # If None, it's assumed to be a 'me' message for self.current_chat
        # If provided, it's a 'them' message for self.current_chat if self.current_chat == sender_is_originator
